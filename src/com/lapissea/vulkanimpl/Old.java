...
// * <a href="https://github.com/glfw/glfw/blob/master/tests/vulkan.c">vulkan</a>
// * test.
// */
...
//		if(errcode!=0){ throw new IllegalStateException(String.format("Vulkan error [0x%X]", errcode)); }
//	}
//
...
//
//	/**
//	 * Return true if all layer names specified in {@code check_names} can be
//	 * found in given {@code layer} properties.
//	 */
...
//			if(required_extensions==null){ throw new IllegalStateException("glfwGetRequiredInstanceExtensions failed to find the platform surface extensions."); }
//
...
//			}else if(err!=0){ throw new IllegalStateException("vkCreateInstance failed. Do you have a compatible Vulkan installable client driver (ICD) installed?"); }
//
...
//
//				/* For tri demo we just grab the first physical physicalDevice */
...
//			}
//
//			/* Look for physicalDevice extensions */
...
//			}
//
//			if(!swapchainExtFound){ throw new IllegalStateException("vkEnumerateDeviceExtensionProperties failed to find the "+VK_KHR_SWAPCHAIN_EXTENSION_NAME+" extension."); }
//
//			if(VALIDATE){
...
//			if(ip.get(0)==0){ throw new IllegalStateException(); }
//
...
//		if(window==NULL){ throw new IllegalStateException("Cannot create a window in which to draw!"); }
//
...
//			if(graphicsQueueNodeIndex==Integer.MAX_VALUE||presentQueueNodeIndex==Integer.MAX_VALUE){ throw new IllegalStateException("Could not find a graphics and a present queue"); }
//
...
//				// If the surface byteSize is undefined, the byteSize is set to the byteSize
...
//				}
//			}else{
//				// If the surface byteSize is defined, the swap chain byteSize must match
...
//
//			// Get the index of the done available swapchain image:
...
//			}
//
//			// Wait for work to export before updating MVP.
